...
class Astar:
    def __init__(self, map, timeAlgorithm, player_pos):
    ...
    def revealNeighbors(self, nodeCoords):
        ...
        for newy in (y - 1, y, y + 1):
            for newx in (x - 1, x, x + 1):
                if self.width > newx > -1 and self.height > newy > -1 and (x, y) != (newx, newy) and (newx, newy) not in [(x + 1, y + 1),(x + 1, y - 1),(x - 1, y + 1),(x - 1, y - 1)]:
