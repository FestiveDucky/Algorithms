...
class Wall(pygame.sprite.Sprite):
    def __init__(self, y, x, group, size, borderless, darkMode):
...
class Maze:
    def __init__(self, BOARDHEIGHT, BOARDWIDTH, SIZE, gamedisplay, display, borderless, darkMode):
        self.SIZE = SIZE
    ...
    def updateDisplay(self, y, x, entire=False, bypass=False):
        if self.display or bypass:
            if not entire:
                if self.darkMode:
                    values = {NOTHING: (20, 20, 20), WALL: (20, 20, 20), PATH: (75, 75, 75),
                              TEMPORARYWALL: (40, 40, 40), START: (0, 255, 0), END: (255, 0, 0)}
                else:
        ...
        for y in range(0, len(self.map)):
            for x in range(0, len(self.map[y])):
                ...
                if self.map[y, x] == WALL:
                    Wall(y, x, self.walls, self.originalSize, self.borderless, self.darkMode)
